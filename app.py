...
def generate_dalle_image(prompt, user_id):
    try:
        if user_id in story_image_urls and prompt in story_image_urls[user_id]:
            return story_image_urls[user_id][prompt]
    ...
    try:
        ...
        if re.search(r"(開始說故事|說個故事|講個故事|說一個故事|講一個故事)", user_text):
            ...
            line_bot_api.reply_message(reply_token, TextSendMessage(
                text="好的，讓我們開始創作一個新的故事吧！主題是「如果我有一個超能力」，你想到的是哪一種超能力呢？"))
            return
        ...
        if re.search(r"(幫我統整|整理|總結|歸納|目前的故事)", user_text):
            ...
            if summary:
                ...
                formatted_summary = "以下是目前的故事內容：\n\n" + summary + \
                    "\n\n故事已經完成了！要不要開始為故事畫插圖呢？" \
                    "我們可以從第一段故事開始，請告訴我你想要如何描繪第一段故事的場景？"
        ...
        # === 寬鬆版繪圖觸發
        DRAW_KEYWORDS = ["畫", "畫畫", "畫一個", "畫一下", "幫我畫", "請畫"]
        if any(k in user_text for k in DRAW_KEYWORDS):
            prompt = user_text
            for k in DRAW_KEYWORDS:
            ...
            if illustration_mode.get(user_id, False) and story_paragraphs.get(user_id):
                if 0 <= current_paragraph < len(story_paragraphs[user_id]):
                    prompt = story_paragraphs[user_id][current_paragraph]
            ...
            if image_url:
                ...
                save_to_firebase(user_id, "user", user_text)
                save_to_firebase(user_id, "assistant", f"[圖片] {image_url}")
            else:
        ...
        if match:
            ...
            if paragraph_match:
                ...
                if paragraph_match.group(0)[1] in chinese_to_number:
                    current_paragraph = chinese_to_number[paragraph_match.group(0)[1]] - 1
                else:
                    current_paragraph = int(paragraph_match.group(0)[1]) - 1
            else:
                current_paragraph = story_current_paragraph.get(user_id, 0)
            ...
            if image_url:
                ...
                if next_paragraph < 5 and user_id in story_paragraphs:
                    ...
                    next_prompt = f"要不要繼續畫第 {next_paragraph + 1} 段故事的插圖呢？\n\n第 {next_paragraph + 1} 段故事內容是：\n{next_story_content}\n\n請告訴我你想要如何描繪這個場景？"
